...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # Note that the scale value is sqrt(key_dimension)
        # Scaling is done to preserve the variance and prevent the dot product from becoming too large
        # Extremee large (small) values will cause the softmax function to converge to one-hot vector
        key_transpose = key.transpose(-2, -1)
        dot_product = query @ key_transpose
        ...
        if attn_mask is not None:
            ...
            # -inf makes softmax 0, so we can use -inf as the additive mask
...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        dot_product = dot_product / ((D // H) ** 0.5)
        ...
        # The shape will be (N, S, H, D/H) -> (N, S, D)
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        pos_embedding = self.encoding(torch.arange(S).to(x.device))
