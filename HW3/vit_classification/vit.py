...
class ViT(nn.Module):
    ...
    def __init__(
        ...
        )  # (learnable [CLS] token embedding), the shape should be (1, 1, d_model) where the first 2 dimensions correspond to the batch and sequence dimensions
    ...
    def patchify(self, images):
        ...
        images = unfold(
            images
        )  # This will patchify the images into shape (N, patch_dim x patch_dim x 3, num_patches)
    ...
    def forward(self, images):
        ...
        output = torch.cat(
            (cls_token, patches_embedded), dim=1
        )  # (append a CLS token to the beginning of the sequence of patch embeddings) -> (N, num_patches + 1, d_model)
        ...
        output = self.fc(
            output[:, 0, :]
        )  # (take the embedding corresponding to the [CLS] token and feed it through a linear layer to obtain the logits for each class)
