...
class Trainer:
    def __init__(
        ...
        self.scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            self.optim, T_0=10, T_mult=1, eta_min=1e-6
        )
    ...
    def eval(self, dataloader):
        ...
        for images, labels in dataloader:
            ...
            correct_predictions += torch.sum(
                predictions == labels.to(self.device)
            ).item()
    ...
    def train(self):
        for i in range(self.num_epochs):
            ...
            if self.verbose and i % self.print_every == 0:
                print(
                    "(epoch %d / %d) loss: %f"
                    % (i, self.num_epochs, self.loss_history[-1])
                )
            ...
            if i % self.save_every == 0:
                torch.save(
                    self.model.state_dict(), "checkpoints/model_{}.pth".format(i)
                )
