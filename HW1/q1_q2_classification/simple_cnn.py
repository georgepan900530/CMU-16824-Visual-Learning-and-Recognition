...
class SimpleCNN(nn.Module):
    ...
    def __init__(self, num_classes=10, inp_size=28, c_dim=1):
        ...
        # Using the formula for convolutional layer output size in https://pytorch.org/docs/stable/generated/torch.nn.Conv2d.html
        self.conv1 = nn.Conv2d(
            c_dim, 32, 5, padding=2
        )  # (inp_size, inp_size) -> (inp_size, inp_size)
        ...
        self.pool1 = nn.AvgPool2d(
            2, 2
        )  # (inp_size, inp_size) -> (inp_size/2, inp_size/2)
        self.pool2 = nn.AvgPool2d(
            2, 2
        )  # (inp_size/2, inp_size/2) -> (inp_size/4, inp_size/4)
