...
class TestDetectorBackboneWithFPN(unittest.TestCase):
    def setUp(self):
        ...
        self.backbone = DetectorBackboneWithFPN(out_channels=self.out_channels).to(
            self.device
        )
    ...
    def test_backbone(self):
        ...
        self.assertEqual(
            self.dummy_fpn_feats["p3"].shape, torch.Size([2, self.out_channels, 28, 28])
        )
        self.assertEqual(
            self.dummy_fpn_feats["p4"].shape, torch.Size([2, self.out_channels, 14, 14])
        )
        self.assertEqual(
            self.dummy_fpn_feats["p5"].shape, torch.Size([2, self.out_channels, 7, 7])
        )

    def test_prediction_network(self):
        self.pred_net = FCOSPredictionNetwork(
            num_classes=self.num_classes, in_channels=64, stem_channels=[64, 64]
        ).to(self.device)
        if self.pred_net.pred_cls is None:
            self.skipTest("FCOSPredictionNetwork is not set up yet.")

        class_logits, boxreg_deltas, centerness_logits = self.pred_net(
            self.dummy_fpn_feats
        )
        if len(class_logits) == 0:
            self.skipTest("Detector backbone not implemented.")

        self.assertEqual(
            class_logits["p3"].shape, torch.Size([2, 784, self.num_classes])
        )
        self.assertEqual(boxreg_deltas["p3"].shape, torch.Size([2, 784, 4]))
        self.assertEqual(centerness_logits["p3"].shape, torch.Size([2, 784, 1]))

        self.assertEqual(
            class_logits["p4"].shape, torch.Size([2, 196, self.num_classes])
        )
        self.assertEqual(boxreg_deltas["p4"].shape, torch.Size([2, 196, 4]))
        self.assertEqual(centerness_logits["p4"].shape, torch.Size([2, 196, 1]))

        self.assertEqual(
            class_logits["p5"].shape, torch.Size([2, 49, self.num_classes])
        )
    ...
    def test_get_fpn_location_coords(self):
        ...
        expected_locations = {
            "p3": torch.tensor(
                [[4.0, 4.0], [4.0, 12.0], [4.0, 20.0], [4.0, 28.0], [4.0, 36.0]]
            ),
            "p4": torch.tensor(
                [[8.0, 8.0], [8.0, 24.0], [8.0, 40.0], [8.0, 56.0], [8.0, 72.0]]
            ),
            "p5": torch.tensor(
                [[16.0, 16.0], [16.0, 48.0], [16.0, 80.0], [16.0, 112.0], [16.0, 144.0]]
            ),
        }
    ...
    def test_gt_targets_for_box_regression(self):
        ...
        output_boxes = fcos_apply_deltas_to_locations(
            _deltas, input_locations, stride=8
        )
    ...
    def test_gt_deltas_invalid_box(self):
        ...
        _deltas = fcos_get_deltas_from_locations(
            input_location, background_box, stride=8
        )
        if _deltas is None:
        ...
        self.assertTrue(torch.equal(_deltas, torch.tensor([[-1.0, -1.0, -1.0, -1.0]])))
        self.assertTrue(
            torch.equal(output_box, torch.tensor([[100.0, 200.0, 100.0, 200.0]]))
        )
    ...
    def test_make_centerness_targets(self):
        ...
        self.assertAlmostEqual(
            rel_error(centerness, expected_centerness), 0.0, places=5
        )

    def test_sigmoid_focal_loss(self):
        dummy_pred_cls_logits = torch.tensor(
            [
                [
                    [-0.0139, -0.7245, -0.5730, -1.7872, 1.5967],
    ...
    def test_centerness_and_regression_loss(self):
        ...
        dummy_gt_boxes = torch.Tensor(
            [[1, 2, 40, 50, 2], [-1, -1, -1, -1, -1]]  # Same GT classes as above cell.
        )
        # shape: (batch_size, num_locations, 4 or 1)
        dummy_pred_boxreg_deltas = torch.Tensor(
            [[[0.1108, -0.2993, -0.6284, 0.8650], [0.5013, 0.5564, -2.0083, 0.4059]]]
        )

        dummy_pred_ctr_logits = torch.Tensor([[[-1.1266], [0.0109]]])
        ...
        if dummy_gt_deltas is None:
            ...
            diff_mean = (
                loss_box
                - torch.tensor(
                    [[0.2145, 0.3092, 0.2196, 0.0756], [0.0000, 0.0000, 0.0000, 0.0000]]
                )
            ).mean()
            self.assertAlmostEqual(diff_mean.cpu().numpy(), 0, places=4)
