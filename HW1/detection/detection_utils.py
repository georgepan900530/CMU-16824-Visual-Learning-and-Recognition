...
def fcos_match_locations_to_gt(
    ...
    Args:
        ...
        gt_boxes: GT boxes of a single image, a batch of `(M, 5)` boxes with
            ...
            codebase, this tensor is directly served by the dataloader. Note that
            M indicates the number of GT boxes in the image.
    ...
    for level_name, centers in locations_per_fpn_level.items():
        ...
        x, y = centers.unsqueeze(dim=2).unbind(
            dim=1
        )  # (H * W, 2) -> (H * W, 2, 1) -> (H * W, 1) (x), (H * W, 1) (y)
        x0, y0, x1, y1 = (
            gt_boxes[:, :4].unsqueeze(dim=0).unbind(dim=2)
        )  # (M, 5) -> (1, M, 5) -> (1, M, 1) (x0), (1, M, 1) (y0), (1, M, 1) (x1), (1, M, 1) (y1)
        ...
        # This step is to check if a center from H*W falls inside any GT box.
        ...
        lower_bound = (
            stride * 4 if level_name != "p3" else 0
        )  # Specifies the smallest objects an anchor on this level can detect.
        upper_bound = (
            stride * 8 if level_name != "p5" else float("inf")
        )  # Specifies the largest objects an anchor on this level can detect.
        match_matrix &= (pairwise_dist > lower_bound) & (pairwise_dist < upper_bound)

        # Match the GT box with minimum area, if there are multiple GT matches. This step aims to mitigate ambiguity problem, where
        # there are overlapping GT boxes
        ...
        match_matrix *= (
            1e8 - gt_areas[:, None]
        )  # smaller area GT boxes will have higher values

        # Find matched ground-truth instance per anchor (un-matched = -1).
        match_quality, matched_idxs = match_matrix.max(
            dim=0
        )  # Get the best match for each anchor
        matched_idxs[match_quality < 1e-5] = -1

        # Anchors with label 0 are treated as background.
        matched_boxes_this_level = gt_boxes[
            matched_idxs.clip(min=0)
        ]  # pick out the matched GT boxes by indexing the correct GT boxes
        matched_boxes_this_level[matched_idxs < 0, :] = -1

        matched_gt_boxes[level_name] = (
            matched_boxes_this_level  # (number of anchors, 5)
        )
...
def fcos_get_deltas_from_locations(
    ...
    N = locations.shape[0]
    xc, yc = torch.unbind(locations, dim=1)  # (N, 2) -> (N, 1) (xc), (N, 1) (yc)
    if gt_boxes.shape[1] == 4:
        x0, y0, x1, y1 = torch.unbind(gt_boxes, dim=1)
    else:
        x0, y0, x1, y1, _ = torch.unbind(gt_boxes, dim=1)

    # L = xc - x0, T = yc - y0, R = x1 - xc, B = y1 - yc. Remember to normalize by stride.
    # Make sure to deal with background boxes, where deltas should be (-1, -1, -1, -1).
    L = (xc - x0) / stride
    T = (yc - y0) / stride
    R = (x1 - xc) / stride
    B = (y1 - yc) / stride
    background_mask = (x0 == -1) | (y0 == -1) | (x1 == -1) | (y1 == -1)
    L[background_mask] = -1
    T[background_mask] = -1
    R[background_mask] = -1
    B[background_mask] = -1
...
def fcos_apply_deltas_to_locations(
    ...
    ##########################################################################
    L, T, R, B = torch.unbind(
        deltas, dim=1
    )  # (N, 4) -> (N, 1) (L), (N, 1) (T), (N, 1) (R), (N, 1) (B)
    L, T, R, B = L * stride, T * stride, R * stride, B * stride

    # Clip the deltas to zero
    L, T, R, B = L.clip(min=0), T.clip(min=0), R.clip(min=0), B.clip(min=0)

    # Compute top-left and bottom-right corners of the boxes
    xc, yc = torch.unbind(locations, dim=1)
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    # )
    ##########################################################################
    L, T, R, B = torch.unbind(deltas, dim=1)
    nominator = torch.min(L, R) * torch.min(T, B)
    denominator = torch.max(L, R) * torch.max(T, B)
    centerness = torch.sqrt(nominator / (denominator + 1e-6))
    background_mask = (L == -1) | (T == -1) | (R == -1) | (B == -1)
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        # According to the paper, we can retrieve the center by using th formula:
        # xc = floor(s/2) + x*s, yc = floor(s/2) + y*s
        # Note that (x, y) is each location on the feature map
        # Do this in matrix form for all locations in the feature map
        H, W = feat_shape[-2], feat_shape[-1]
        x = torch.arange(W, dtype=dtype, device=device)
        y = torch.arange(H, dtype=dtype, device=device)
        cord_x, cord_y = torch.meshgrid(x, y, indexing="xy")
        cord_xc = level_stride // 2 + cord_x * level_stride
        cord_yc = level_stride // 2 + cord_y * level_stride
        location_coords[level_name] = torch.stack((cord_yc, cord_xc), dim=-1).reshape(
            -1, 2
        )
        ######################################################################
        #                             END OF YOUR CODE                       #
