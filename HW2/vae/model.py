...
class Encoder(nn.Module):
    ...
    def __init__(self, input_shape, latent_dim):
        ...
        self.convs = nn.Sequential(
            nn.Conv2d(C, 32, kernel_size=3, stride=1, padding=1),  # (H, W) -> (H, W)
            nn.ReLU(),
            nn.Conv2d(
...
class VAEEncoder(Encoder):
    def __init__(self, input_shape, latent_dim):
        ...
        ##################################################################
        C, H, W = input_shape
        self.fc = nn.Linear(C * H * W, 2 * latent_dim)
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class Decoder(nn.Module):
    ...
    def forward(self, z):
        ...
        ##################################################################
        out = self.fc(z)
        out = out.reshape(-1, 256, self.output_shape[1] // 8, self.output_shape[2] // 8)
