...
class UpSampleConv2D(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        # Step 1: (batch, channel, height, width) -> (batch, channel*upscale_factor^2, height, width)
        # This step is for the following PixelShuffle operation, where I've checked in:
        # https://pytorch.org/docs/stable/generated/torch.nn.PixelShuffle.html
        x = torch.repeat_interleave(x, int(self.upscale_factor**2), dim=1)

        # Step 2: Apply PixelShuffle
...
class DownSampleConv2D(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        # Step 2: Split channel-wise and reshape
        chunks = torch.chunk(x, int(x.shape[1] // (self.downscale_ratio**2)), dim=1)
        x = torch.stack(chunks, dim=2)
        x = x.permute(1, 0, 2, 3, 4)

        # Step 3: Take the average across dimension 0, apply conv
...
class ResBlockUp(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        # to the layer output.
        ##################################################################
        upsampled_x = self.upsample_residual(x)
        out = self.layers(x)
        return out + upsampled_x
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class ResBlockDown(torch.jit.ScriptModule):
    ...
    def forward(self, x):
        ...
        ##################################################################
        downsampled_x = self.downsample_residual(x)
        out = self.layers(x)
        return out + downsampled_x
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
...
class ResBlock(torch.jit.ScriptModule):
    ...
    def __init__(self, input_channels, kernel_size=3, n_filters=128):
        ...
        self.layers = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(
                n_filters, n_filters, kernel_size=kernel_size, stride=1, padding=1
            ),
        )
...
class Generator(torch.jit.ScriptModule):
    ...
    def forward_given_samples(self, z):
        ...
        generated_images = self.layers(x)
