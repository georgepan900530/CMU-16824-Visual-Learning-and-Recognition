...
def get_optimizers_and_schedulers(gen, disc):
    ...
    scheduler_discriminator = torch.optim.lr_scheduler.LambdaLR(
        optim_discriminator, lr_lambda=lambda x: lr_lambda(x, 500000)
    )
    scheduler_generator = torch.optim.lr_scheduler.LambdaLR(
        optim_generator, lr_lambda=lambda x: lr_lambda(x, 100000)
    )
    ##################################################################
    #                          END OF YOUR CODE                      #
    ##################################################################
    ...
    while iters < num_iterations:
        for train_batch in train_loader:
            with torch.cuda.amp.autocast(enabled=amp_enabled):
                ...
                # According to the WGAN paper, the interpolated data is
                # interp = epsilon * real_data + (1 - epsilon) * fake_data
                # where epsilon is sampled from a uniform distribution between 0 and 1
                epsilon = torch.rand(batch_size, 1, 1, 1).cuda()
                interp = epsilon * train_batch + (1 - epsilon) * fake_images
            ...
            if iters % log_period == 0 and iters != 0:
                with torch.no_grad():
                    with torch.cuda.amp.autocast(enabled=amp_enabled):
                        ...
                        generated_samples = (generated_samples.data + 1) / 2.0
                        ##################################################################
                        #                          END OF YOUR CODE                      #
                        ##################################################################
                    save_image(
                    ...
                    if os.environ.get("PYTORCH_JIT", 1):
                        ...
                        torch.jit.save(
                            torch.jit.script(disc), prefix + "/discriminator.pt"
                        )
                    else:
