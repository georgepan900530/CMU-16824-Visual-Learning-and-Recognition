...
class DiffusionModel(nn.Module):
    def __init__(
        ...
        # According to DDPM, we need alpha_bar = cumulative product of alpha
        self.alphas_cumprod = torch.cumprod(alphas, dim=0)
        self.alphas_cumprod_prev = torch.cat(
            [torch.tensor([1.0], device=alphas.device), self.alphas_cumprod[:-1]]
        )
        ...
        self.x_0_pred_coef_2 = -torch.sqrt(1 - self.alphas_cumprod) / torch.sqrt(
            self.alphas_cumprod
        )
        ...
        self.posterior_mean_coef2 = (
            torch.sqrt(alphas) * (1 - self.alphas_cumprod_prev)
        ) / (1 - self.alphas_cumprod)
        ...
        self.posterior_variance = (
            (1 - self.alphas_cumprod_prev) * self.betas / (1 - self.alphas_cumprod)
        )
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
    ...
    def sample_ddpm(self, shape, z):
        ...
        for t in tqdm(range(self.num_timesteps - 1, 0, -1)):
            batched_times = torch.full(
                (img.shape[0],), t, device=self.device, dtype=torch.long
            )
    ...
    def ddim_step(
        ...
        tau_i = torch.Tensor([tau_i]).to(device).long()
        tau_isub1 = torch.Tensor([tau_isub1]).to(device).long()
        ...
        beta_hat_tau_i = (
            (1 - alpha_tau_isub1) * self.betas[tau_isub1] / (1 - alpha_tau_i)
        )
        var_tau_i = eta * beta_hat_tau_i
        sigma_tau_i = torch.sqrt(var_tau_i)

        # Step 4: Compute the coefficient of \epsilon_{\tau_{i}}
        epsilon_coef = torch.sqrt(1 - alpha_tau_isub1 - var_tau_i)
