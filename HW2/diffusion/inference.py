...
def get_fid(gen, dataset_name, dataset_resolution, z_dimension, batch_size, num_gen):
    ...
    gen_fn = (
        lambda z: (
            gen.sample_given_z(z, (-1, 3, dataset_resolution, dataset_resolution)) + 1
        )
        / 2.0
        * 255
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num-images", default=100, type=int, help="Number of images per iteration"
    )
    parser.add_argument(
        "--image-size", default=32, type=int, help="Image size to generate"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=25,
        help="Number of timesteps to sample for DDIM",
    )
    ...
    with torch.no_grad():
        ...
        if args.compute_fid:
            ...
            score = get_fid(
                diffusion, "cifar10", 32, 32 * 32 * 3, batch_size=256, num_gen=10_000
            )
            print("FID: ", score)
